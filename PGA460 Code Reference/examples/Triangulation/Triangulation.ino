/*------------------------------------------------- GetDistance -----
 PROJECT:     PGA460 UART & OWU Bus Triangulation Demo
 DESCRIPTION: Performs triangulation calculation based on tne law of 
              cosines. Two independent receiver elements are required
              to capture the leg length information for all sides of 
              the virtual triangle. This example uses address '0b000'
              as the transceiver module, and address '0b001' as the
              receive-only module. Do not connect the transducer to
              the driver pins (OUTA/OUTB) on the receive-only module.
...
// Configuration variables
  byte commMode = 0;            // Communication mode: 0=UART, 2=OneWireUART
...
// BusDemo &  Triangulation exclusive variables
  byte numDevices = 2;          // number of PGA460 devices connected to the bus
  ...
  double distanceA = 0;         // distance (m) from sensor module 1 (S1) to the targeted object based on UMR result
  double distanceB = 0.1;    	// distance (m) between sensor module 1 (S1) and sensor module 2 (S2) - assumed to be fixed
  double distanceC = 0;         // distance (m) from sensor module 2 (S2) to the targeted object based on UMR result
  double inAngle = 0;           // angle (degrees) from transceiver element S1 to the targeted object
...
void initPGA460() {
      ...
      int inByte = 0;         // incoming serial byte
        ...
        delay(1000);
        Serial.println("                                 PGA460-Q1 UART & OWU Triangulation Demo                                 ");
        ...
        Serial.println("--- Input can be entered as a single string to auto-increment/fill each request. E.g. 003xxx5001x00");
    ...
    for (int i=0; i<numInputs; i++)
    {
        switch(i)
        {
          ...
          case 10: Serial.print("11. Distance Between S1 and S2 Modules: 0-9 * 2.54cm, x=Skip ... "); break;
          case 11: Serial.print("12. OWU Ready?: 0=Yes/NotUsed, 1=No,UART-Config, 2=No,IO-Config ... "); break;
        ...
        if (inByte != 115-48 && inByte != 113-48 && inByte != 120-48) // if input is neither 's', 'q', or 'x'
        {  
          delay(300);
          ...
          switch(i)
          {
            ...
            case 10: distanceB = inByte * 0.0254; break;     
...
void loop() {                 // put your main code here, to run repeatedly
  ...
  delay(1000);
  Serial.println("PGA460-Q1 Dual-Sensor Triangulation");
  #endif
  Serial.println("___________________________________");
  Serial.println("ADDR 0 |    1                      ");
  Serial.println("-------|--------");
  delay(100);
  
  while(1){

        // -+-+-+-+-+-+-+-+-+-+-  PRESET 2 BURST-AND-LISTEN BROADCAST  -+-+-+-+-+-+-+-+-+-+- //
      
      ussc.ultrasonicCmd(18, 1); //Preset 2 burst-and-listen Broadcast for 1 Obj

      for(int i = 0; i < numDevices ; i++)
      {
        // -+-+-+-+-+-+-+-+-+-+-  PULL ULTRASONIC MEASUREMENT RESULTS   -+-+-+-+-+-+-+-+-+-+- //
          distance = 0.00;
          uartAddrUpdate = i;
          ...
          switch (i)
          {
            case 0: // log the distance value from S1 to object for triangulation equation
				distanceA = distance; break;                            
            case 1: // log the distance value from S2 to object for triangulation equation.
              if(distanceA>=distance)
              {
                distanceC = distance - (distanceA - distance); 
              }
              else
              {
                distanceC = distance + (distance - distanceA);
              }
              break;   
            default: break;          
          }
        ...
        // -+-+-+-+-+-+-+-+-+-+-  SERIAL FORMATTING   -+-+-+-+-+-+-+-+-+-+- //
          ...
          for (int j = 0; j < dSize ;j++){Serial.print(" ");}        
          if(i == numDevices-1){Serial.print("     ");}
          else{Serial.print(" ");}
      ...
      // -+-+-+-+-+-+-+-+-+-+-  TRIANGULATION USING LAW OF COSINES   -+-+-+-+-+-+-+-+-+-+- //      
      inAngle = ussc.triangulation(distanceA, distanceB, distanceC); // returns internal angle from transciever to targeted object
      if (inAngle == 360 || distanceA >= 11.2 || distanceC >= 11.2)
      {
        // No object detected. inAngle of 360 denotes error case
      }
      else
      {
        Serial.print("Object position (degrees) = "); Serial.print(inAngle); // only print angle information for valid pair of measurements
      }
      Serial.println("");
